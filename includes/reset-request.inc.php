...
if(isset($_POST['reset-req-submit'])) {
    ...
    $sql = "DELETE FROM `pwdreset` WHERE `pwd_reset_email`='$user_email';";
    ...
    $sql = "INSERT INTO `pwdreset` (`pwd_reset_email`,`pwd_reset_selector`,`pwd_reset_token`,`pwd_reset_expires`) VALUES ('$user_email','$selector','$token_hash','$expires');";
    $sqlReset = mysqli_query($conn,$sql);
    //$stmt = mysqli_stmt_init($conn);
    //if(!mysqli_stmt_prepare($stmt,$sql)) {
    if(!$sqlReset){
        header('Location: ../reset-pwd.php?err=sqlerr');     
