...
if(isset($_POST['signup_submit'])) {
    ...
    if(!filter_var($email_id,FILTER_VALIDATE_EMAIL)) {
        ...
        $email_sql = 'SELECT `email` FROM `users` WHERE `email`="$email_id"';
        //$stmt = mysqli_stmt_init($conn);
        //if(!mysqli_stmt_prepare($stmt,$username_sql)) {
        if(!mysqli_query($conn,$username_sql)){
            ...
            */
            ...
            $sqlResult = mysqli_query($conn,$username_sql);
            $usernameCount = mysqli_num_rows($sqlResult);
            if($usernameCount > 0) {
                ...
                if(!mysqli_query($conn,$email_sql)){
                    ...
                    */
                    $sqlResult = mysqli_query($conn,$email_sql);
                    $emailCount = mysqli_num_rows($sqlResult);
                    if($emailCount > 0) {
                        header('Location: ../register.php?error=emailexists');
                        exit();   
                    } else {
                        /*
                        $sql = 'INSERT INTO `users` (`username`,`email`,`password`) VALUES ("$username","$email_id","$password")';
                        //$stmt = mysqli_stmt_init($conn);
                        ...
                        */
                            ...
                            mysqli_stmt_execute($stmt);
                            */
                            $sql = "INSERT INTO `users` (`username`,`email`,`password`) VALUES ('$username','$email_id','$pwd_hash');";
                            $sqlResult = mysqli_query($conn,$sql);
                            header("location: ./../login.php");

                            
                            // LOGIN USer                            
                            $sql = 'SELECT * FROM `users` WHERE `username`="$username" AND `email`="$email_id";';
                            //$stmt = mysqli_stmt_init($conn);
                            //if(!mysqli_stmt_prepare($stmt,$sql)) {
                            if(!mysqli_query($conn,$sql)){
