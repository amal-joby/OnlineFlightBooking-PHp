...
td {
  ...
  color: #424242;
         ...
         if(isset($_POST['search_but'])) {
          ...
          <div class="container-md mt-2">
            ...
            <table class="table table-striped table-bordered table-hover">
              ...
              <tbody>
                <?php
                $sql = "SELECT * FROM `flight` WHERE `source`='$dep_city' AND `destination`='$arr_city' AND DATE(`departure`)='$dep_date' ORDER BY `price` ;";
                $sqlResult = mysqli_query($conn,$sql);

                /*
                $sql = 'SELECT * FROM `flight` WHERE  `Destination` ="$arr_city" AND `source`="$dep_city"   ORDER BY `Price`';
                $sqlResult = mysqli_query($conn,$sql);
	...
	<p class="text-light text-center">&copy; <?php echo date('Y');?> - Developed By Nintu Varghese,Mariya Joy & Grace Mariya</p>
</footer>